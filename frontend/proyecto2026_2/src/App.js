                  ...
                  <article className="menu-card" key={id || name}>
                    ...
                    <div className="menu-info">
                      ...
                      <div className="price-row">
                        <strong>{price}</strong>
                        <button className="mini-action" type="button">Agregar</button>
                      </div>
                    </div>
                  </article>
                ))}
          ...
          {locations.map(([district, address]) => (
            <article className="location-card" key={district}>
              <div className="location-info">
                ...
                <button className="mini-action" type="button" onClick={() => onViewChange('reserva')}>Reservar</button>
              </div>
...
function Reservation({ onAddBooking }) {
  ...
  const selectedTable = tables.find((table) => String(table.idmesa) === selectedTableId);
    ...
    <section className="view active">
      <div className="page-shell reservation-layout">
        ...
        <form className="reservation-form" onSubmit={handleSubmit}>
          ...
          <label>Mesa
            <select name="mesaId" required disabled={!tables.length} value={selectedTableId} onChange={(event) => setSelectedTableId(event.target.value)}>
              <option value="">{tables.length ? 'Selecciona una mesa' : 'No hay mesas disponibles'}</option>
              {tables.map((table) => (
                <option key={table.idmesa} value={table.idmesa}>
                  {table.nombremessa} ({table.cantidadsillas} sillas)
                </option>
              ))}
            </select>
          </label>
        ...
        <section className="admin-dashboard">
          ...
          {tab === 'inventario' && (
            <section className="admin-panel active">
              ...
              <form className="reservation-form" onSubmit={async (event) => {
                ...
                <div className="two-columns"><label>Cantidad<input name="quantity" type="number" min="0" required /></label><label>Unidad<input name="unit" placeholder="unidades" /></label></div>
          ...
          {tab === 'delivery' && (
            <section className="admin-panel active">
              ...
              <AdminCardList data={deliveries} actionLabel="Actualizar estado" onAction={(index) => {
                const [name, order, district, status, id] = deliveries[index];
                const nextStatus = window.prompt(`Estado de ${order}`, status);
                if (nextStatus) onUpdateDelivery(id, { customer_name: name, order_reference: order, district, status: nextStatus });
              }} onDelete={(index) => onDeleteDelivery(deliveries[index][4])} />
            </section>
          )}
...
function App() {
  ...
  useEffect(() => {
    fetch(apiUrl('/api/reservations'))
      ...
      .then((reservations) => setBookingsState(reservations.map((reservation) => [
        ...
        `${reservation.guests} ${reservation.guests === 1 ? 'persona' : 'personas'} - Tel: ${reservation.phone}`,
  ...
  async function addBooking(reservation) {
    ...
    const booking = [
      saved.guest_name,
      `${saved.guests} ${saved.guests === 1 ? 'persona' : 'personas'} - Tel: ${saved.phone}`,
